# Add these to your existing utils.py file

def log_agent_title_html(agent_name: str, emoji: str = "🤖"):
    """Log agent start with styled formatting."""
    print(f"\n{'=' * 80}")
...
def log_tool_call_html(tool_name: str, arguments: str):
    """Log a tool call."""
    print(f"\n🔧 TOOL CALL: {tool_name}")
    print(f"   Arguments: {arguments[:200]}{'...' if len(arguments) > 200 else ''}")
...
def log_tool_result_html(result):
    ...
    if isinstance(result, dict):
        result_str = json.dumps(result, indent=2)[:500]
    else:
        result_str = str(result)[:500]
    print(f"\n✅ TOOL RESULT:")
    print(f"   {result_str}{'...' if len(str(result)) > 500 else ''}")


def log_final_summary_html(content: str):
    """Log final agent summary."""
    print(f"\n{'─' * 80}")
    print("📋 AGENT SUMMARY:")
    print(f"{'─' * 80}")
...
def log_unexpected_html():
    ...
    print("\n⚠️ UNEXPECTED: No tool calls or content returned")
